...
(function (window, document) {
  ...
  // Collects player instances
  // ----------------------------------------------------
  window.bplayers = document.getElementsByTagName('bplayer');
  ...
  var videoBPlayer = {
    ...
    // ----------------------------------------------------
    init: function () {
      for (p = 0; p < bplayers.length; ++p) {
        this.makeChrome(bplayers[p]);
        this.makeControls(bplayers[p]);
        this.handleActions(bplayers[p]);
      }
    },
    // ----------------------------------------------------
    // Create player chrome
    ...
    makeChrome: function (element) {
      ...
      // this.getURL(files);
      this.getURL('https://www.bitchute.com/video/CdeY4sM4IKMy');
    },
    // ----------------------------------------------------
    // Create player chrome
    // ----------------------------------------------------
    getURL: function (source) {
      var videoid, baseurl, page, url;
      baseurl = 'https://www.bitchute.com/video/';
      videoid = 'CdeY4sM4IKMy';
      link = baseurl + videoid;
  
      xhr = new XMLHttpRequest();
      xhr.onreadystatechange = function () {
        // Handle error
        if (xhr.status !== 200) {
          console.log("There is something wrong with server response!");
    ...
    handleActions: function (chrome) {
      ...
      video.addEventListener('loadedmetadata', function () {
        ...
        videoBPlayer.stylize(chrome, fg, bg);
        videoBPlayer.colorize(volume, fg, bg);        
        clock.innerHTML = videoBPlayer.formatTime(this.duration);
      }, false);

      // Playing
      video.addEventListener('play', function () {
        chrome.setAttribute('class', 'player playing');
        videoBPlayer.progress(this);
      }, false);

      // Paused
      video.addEventListener('pause', function () {
        chrome.setAttribute('class', 'player paused');
        videoBPlayer.stopProgress();
      }, false);
      ...
      full.addEventListener('click', function(e) {
        videoBPlayer.handleFullscreen(chrome);
      });
      ...
      volume.onchange = function () {
        ...
        videoBPlayer.colorize(this, fg, bg);
      };
    },
    // ----------------------------------------------------
    // Play / pause video
    ...
    seekToTime: function(el, video, fg, bg) {
      el.addEventListener('change', function (e) {
        ...
        videoBPlayer.colorize(e.target, fg, bg);
      }, false);
    },
    // ----------------------------------------------------
    // Check for fullscreen state
    ...
    progress: function (video) {
      (function progressTrack() {
        videoBPlayer.updateTime(video);
        videoBPlayer.updateProgressBar(video);
